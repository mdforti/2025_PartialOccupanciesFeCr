...
In order to keep the size of the simulation to one unit cell, we consider two approximations to the partial occupancy, in both cases disregarding Cr occupation in site 2a and rounding occupancy to integer numbers.  
A first model keeps full occupancy of Fe in the sites 2\textit{a} and 8\textit{i\textsubscript{2}}, full Cr occupancy in site 4\textit{f}, but sites 8\textit{i\textsubscript{1}} and 8\textit{j} will have 5 Cr and 3 Fe atoms per unit cell. We name this model PO1. 
...
A second further transfers one Cr atom from 4\textit{f} to 8\textit{i\textsubscript{2}}.
...
As show in \autoref{fig:PartialOccupancies}, there are three possible configurations with nominal composition Fe16Cr14 ($\approx$ 52at\% Fe). 
...
In the other hand, the triclinic unit cell has 21 independent components where none of them is necesarily zero or equal to another. 
This corresponds to the upper triangular part, as the elastic tensor is symmetric by definition.
...
\MFnew{The formation enthalpies of the PO1 set approaches the Gibbs free energies from the CEF at 0K, even for our limited selection of samples, and to the moment we dont find any sample with energies below this limit.
...
Here we intend to use this potential as a screening tool to scan the full configurational space and select the most relevant configurations for further DFT calculations, but it does not necesarilly need to be highly accurate. 
The DFT extended dataset is ilustrated in \autoref{fig:DFTDataset} together with a diagram of our workflow, while the final potential performance is reported in \autoref{tab:MLIPPerformance}.
